/*
  # Fix Episode Settings Comparison - Type Cast Issue

  1. Changes
    - Fixes type mismatch where SUM returns numeric instead of bigint
    - Explicitly casts total_monthly_projected_views to bigint
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      (SELECT SUM((ch->>'monthlyProjectedViews')::bigint)::bigint FROM jsonb_array_elements(eps.distribution_channels) ch WHERE (ch->>'enabled')::boolean = true),
      0::bigint
