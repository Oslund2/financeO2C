/*
  # Fix Permanent Deletion System Audit Log Columns

  This migration fixes critical bugs in the workspace deletion system where
  the functions were trying to insert into audit_log with incorrect column names.
  ...
  ## Column Mapping
  - organization_id → removed (stored in metadata instead)
  - action → action_type
  - old_values/new_values → changes
  - Additional metadata moved to metadata field
  - Added required entity_type and entity_id fields
*/

-- Function to schedule organization deletion (FIXED)
...
BEGIN
  ...
  -- Log the action to audit_log (FIXED COLUMNS)
  INSERT INTO audit_log (
    ...
    jsonb_build_object(
      'scheduled_deletion_at', v_scheduled_date,
      'deletion_confirmed_at', now(),
      'previous_scheduled_deletion_at', null
    ),
    jsonb_build_object(
...
-- Function to cancel scheduled deletion (FIXED)
...
BEGIN
  ...
  SET
    ...
    archived = CASE WHEN restore_from_archive THEN false ELSE archived END,
    updated_at = now()
  WHERE id = org_uuid;

  -- Log the cancellation (FIXED COLUMNS)
  INSERT INTO audit_log (
    user_id,
    action_type,
    entity_type,
    ...
    jsonb_build_object(
      ...
      'previous_deletion_confirmed_at', v_org.deletion_confirmed_at
    ),
    jsonb_build_object(
      'organization_id', org_uuid,
      'organization_name', v_org.name,
...
-- Function to permanently delete organization (FIXED)
...
BEGIN
  ...
  -- Create final audit log entry before deletion (FIXED COLUMNS)
  INSERT INTO audit_log (
    user_id,
    action_type,
    entity_type,
