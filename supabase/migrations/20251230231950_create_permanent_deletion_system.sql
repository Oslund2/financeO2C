/*
  # Permanent Workspace Deletion System

  This migration implements a comprehensive permanent deletion system with multiple safety valves:
  - 7-day grace period after scheduling deletion
  - Owner-only permissions for all deletion operations
  - Requires workspace to be archived first before deletion can be scheduled
  - Double opt-in confirmation tracked in database
  - All actions logged to audit trail

  1. Schema Changes
    - Add `scheduled_deletion_at` column to organizations table
    - Add `deletion_confirmed_at` column to track confirmation timestamp
    - Add index for efficient querying of pending deletions

  2. New Functions
    - `schedule_organization_deletion`: Schedule deletion with 7-day grace period
    - `cancel_organization_deletion`: Cancel a scheduled deletion
    - `permanently_delete_organization`: Execute permanent deletion after grace period
    - `get_deletion_status`: Get current deletion scheduling status

  3. Security
    - All functions restricted to organization owner only
    - Workspace must be archived before deletion can be scheduled
    - Grace period enforcement in permanent deletion function

  4. Audit Trail
    - All deletion-related actions logged with full metadata
    - Captures user ID, timestamps, and action details
*/

-- Add deletion scheduling columns to organizations
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'organizations' AND column_name = 'scheduled_deletion_at'
  ) THEN
    ALTER TABLE organizations ADD COLUMN scheduled_deletion_at timestamptz;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'organizations' AND column_name = 'deletion_confirmed_at'
  ) THEN
    ALTER TABLE organizations ADD COLUMN deletion_confirmed_at timestamptz;
  END IF;
END $$;

-- Create index for efficient querying of pending deletions
CREATE INDEX IF NOT EXISTS idx_organizations_scheduled_deletion 
  ON organizations(scheduled_deletion_at) 
  WHERE scheduled_deletion_at IS NOT NULL;

-- Add comments for documentation
COMMENT ON COLUMN organizations.scheduled_deletion_at IS 
  'Timestamp when permanent deletion is scheduled to occur (7 days after confirmation)';
COMMENT ON COLUMN organizations.deletion_confirmed_at IS 
  'Timestamp when the owner confirmed the deletion request';
...
BEGIN
  ...
  INSERT INTO audit_log (
    ...
    'organization',
    org_uuid,
    jsonb_build_object('scheduled_deletion_at', null),
    jsonb_build_object(
      'scheduled_deletion_at', v_scheduled_date,
      'deletion_confirmed_at', now()
    ),
...
BEGIN
  ...
  INSERT INTO audit_log (
    ...
    jsonb_build_object(
      'scheduled_deletion_at', v_org.scheduled_deletion_at,
      'deletion_confirmed_at', v_org.deletion_confirmed_at
    ),
    jsonb_build_object(
      'scheduled_deletion_at', null,
      'deletion_confirmed_at', null,
...
CREATE OR REPLACE FUNCTION get_organization_deletion_status(org_uuid uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF v_org.scheduled_deletion_at IS NOT NULL THEN
    v_time_remaining := v_org.scheduled_deletion_at - now();
    v_can_delete_now := v_org.scheduled_deletion_at <= now();
  ELSE
    v_time_remaining := NULL;
    v_can_delete_now := false;
  END IF;

  RETURN jsonb_build_object(
    'exists', true,
    'is_archived', v_org.archived_at IS NOT NULL,
    'archived_at', v_org.archived_at,
    'is_deletion_scheduled', v_org.scheduled_deletion_at IS NOT NULL,
    'scheduled_deletion_at', v_org.scheduled_deletion_at,
    'deletion_confirmed_at', v_org.deletion_confirmed_at,
    'time_remaining_seconds', CASE 
      WHEN v_time_remaining IS NOT NULL AND v_time_remaining > interval '0' 
      THEN EXTRACT(EPOCH FROM v_time_remaining)::int
      ELSE 0
    END,
    'can_delete_now', v_can_delete_now,
    'grace_period_days', 7
  );
END;
$$;
...
GRANT EXECUTE ON FUNCTION schedule_organization_deletion(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION schedule_organization_deletion(uuid, uuid) TO anon;
GRANT EXECUTE ON FUNCTION cancel_organization_deletion(uuid, uuid, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION cancel_organization_deletion(uuid, uuid, boolean) TO anon;
GRANT EXECUTE ON FUNCTION permanently_delete_organization(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION permanently_delete_organization(uuid, uuid) TO anon;
GRANT EXECUTE ON FUNCTION get_organization_deletion_status(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION get_organization_deletion_status(uuid) TO anon;
