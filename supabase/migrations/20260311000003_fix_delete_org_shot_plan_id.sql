/*
  # Fix immediately_delete_organization: column "production_shot_plan_id" does not exist

  ## Problem
  The `immediately_delete_organization` function tried to delete from
  `production_shot_prompts` using a column named `production_shot_plan_id`,
  but the actual FK column is `shot_plan_id`. This caused every workspace
  deletion to fail with: "column "production_shot_plan_id" does not exist"

  ## Fix
  Replace the function with a corrected version using the correct column name
  `shot_plan_id` when deleting from `production_shot_prompts`.
*/
