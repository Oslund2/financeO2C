/*
  # Fix delete_series Function - Remove Invalid workflow_notifications References

  ## Problem
  The delete_series function tries to delete from workflow_notifications using 
  workflow_instance_id, but that column doesn't exist in workflow_notifications.
  
  The workflow_notifications table only has workflow_template_id, not workflow_instance_id.

  ## Solution
  Since workflow_notifications has ON DELETE CASCADE from workflow_templates,
  and workflow_templates references series_id with ON DELETE CASCADE,
  we can simply delete workflow_templates and let CASCADE handle workflow_notifications.
  
  Remove the incorrect DELETE statements for workflow_notifications.

  ## Tables Structure
  - workflow_notifications.workflow_template_id → workflow_templates.id (ON DELETE CASCADE)
  - workflow_templates.series_id → series.id (ON DELETE CASCADE)
  - workflow_escalations.workflow_instance_id → workflow_instances.id
  - workflow_escalations.approval_request_id → approval_requests.id
  - workflow_escalations.workflow_stage_id → workflow_stages.id
*/
...
BEGIN
  ...
  -- workflow_escalations must be deleted before approval_requests and workflow_stages
  ...
  -- Delete workflow_templates - this will CASCADE to workflow_notifications
