/*
  ...
  ## Problem
  The delete_series function tries to delete from production_jobs using a non-existent
  'series_id' column. The production_jobs table uses 'entity_id' and 'entity_type' columns instead.

  ## Solution
  Update the DELETE statement to use the correct columns:
  - Change: DELETE FROM production_jobs WHERE series_id = series_uuid
  - To: DELETE FROM production_jobs WHERE entity_type = 'series' AND entity_id = series_uuid
*/
...
BEGIN
  ...
  -- FIXED: Use entity_type and entity_id columns instead of series_id
  ...
  DELETE FROM template_installations WHERE target_series_id = series_uuid;
