/*
  # Fix Episode Progress Table References

  ## Issue
  Multiple database functions reference a non-existent table `episode_progress`.
  The actual table in the schema is `episode_progress_milestones`.

  ## Changes
  1. Update `delete_script_cascade` function to remove invalid table reference
  2. Update `permanent_delete_organization_data` function to remove invalid table reference
  
  ## Notes
  - The `episode_progress_milestones` table has ON DELETE CASCADE to episodes
  - Explicit deletion of episode_progress_milestones is already handled in the functions
  - Removing redundant/invalid episode_progress deletions
*/

-- Fix delete_script_cascade function
...
BEGIN
  ...
  -- REMOVED: DELETE FROM episode_progress (table does not exist)
  -- episode_progress_milestones handles all progress tracking
