    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
      ...
      <div className="px-6 py-4 bg-gradient-to-r from-violet-600 to-purple-700 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-purple-100">AI concept evaluation before a single frame is produced</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
          ...
          {!showChannelPicker && selectedChannels.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {selectedChannels.map(ch => (
                <span key={ch} className="px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded-full">{ch}</span>
              ))}
            </div>
          )}
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
        >
